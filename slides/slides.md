<!-- .slide: <%= bg("unsplash-Jztmx9yqjBw-stars.jpg") %> id="title" -->
# Hope in the Coming Messiah
## Isaiah 9:2-7

>>>
+ **Help**, Lord!
  + see **brokenness**, corruption,
  + injustice, violence
+ war on **global** scale:
  + Gaza, Ukraine, Venezuela, Sudan, Xinjiang
+ and on **personal** scale:
  + esp **holidays**
  + reminder of **broken** rl
  + old **wounds** never healed

---
<!-- .slide: data-background="white" -->
# **Why**, Lord, <br/> do You not **save**?

<!-- .element: class="r-fit-text" -->

>>>
+ **Fix** things!
  + make everyone **happy**?
+ longing for **justice** + peace
+ but when see God's **judgment**:
  + "wait, not like **that**!"

---
<!-- .slide: <%= bg("unsplash-TStNU7H4UEE-hand_sunset.jpg") %> class="outline" id="outline" -->
## Hope in the Coming Messiah
1. The **Darkness** in our World [(8:19-22)](# "ref")
1. The **Light** Shining [(9:2-7)](# "ref")

[Montreal, CA. Marc-Olivier Jodoin](https://unsplash.com/photos/TStNU7H4UEE "caption")

>>>
+ ch9 **familiar**, but context in
+ ch7-8: what **darkness** that
  + Chr **rescues** us from?
+ Has the light **already** come,
  + or still **yet** to come?
+ Not just abt **Christmas**
  + for **today**, and until Chr returns

---
<!-- .slide: <%= bg("flickr-noamchen-jerusalem.jpg") %> -->
In the days of **Ahaz** the son of Jotham, <br/>
son of Uzziah, king of Judah, 

Rezin the king of **Syria** and <br/>
Pekah the son of Remaliah the king of **Israel**

came up to **Jerusalem** to wage war against it, <br/>
but could not yet mount an attack against it. 

[Isaiah 7:1 (ESV)](# "ref")
[Noam Chen, Israeli Ministry of Tourism.  CC BY-ND 2.0](https://www.flickr.com/photos/israelphotogallery/15002111788/ "caption")

>>>
+ God **called** Isaiah when **Uzziah** died:
  + end of long period of **peace**
+ Jotham + Ahaz: corrupt, **rebel** to God
+ 2 main **crises** during Isaiah's time:
  + Sennacherib's **siege** of Jeru
  + during Hezekiah's reign, later
+ and **this** threat from 2 N. kingdoms:

---
![Assyria, Aram, Israel, Judah](/img/bibletalk-kings-map.png)

[BibleTalk, CC-BY-NC-SA 3.0](https://bibletalk.tv/graphics/aram-israel-judah-assyria "caption")

>>>
+ prophet Isaiah + king Ahaz in **Judah**
  + capital city is Jerusalem
+ N. kingdom of **Israel** / Ephraim:
  + Samaria, Pekah son of Remaliah
+ to NE is **Aram**/Syria:
  + Damascus, Rezin
+ further NE is **Assyria**

---
<!-- .slide: <%= bg("unsplash-50bzI1F6urA-forest.jpg") %> -->
When the house of David was told, <br/>
“Syria is in league with Ephraim,” 

the **heart** of Ahaz and the heart of his people <br/>
**shook** as the trees of the forest <br/>
shake before the wind.

[Isaiah 7:2 (ESV)](# "ref")
[Kerkini Lake, Greece. Eimaiokanenas, CC-BY-SA-4.0](https://commons.wikimedia.org/wiki/File:Kerkini_Lake_Port.jpg "caption")

>>>
+ **powerless** before overwhelming opp
+ but God promised imminent **justice**:
  + before **baby** learns to talk,

---
<!-- .slide: <%= bg("wm-Ashurbanipal.jpg") %> -->
the wealth of **Damascus** and the spoil of **Samaria**
will be carried away before the king of **Assyria**.

[Isaiah 8:4 (ESV)](# "ref")
[Ashurbanipal, detail of lion-hunt scene](https://commons.wikimedia.org/wiki/File:Ashurbanipal,_detail_of_a_lion-hunt_scene_from_Nineveh,_7th_century_BC,_the_British_Museum.jpg "caption")

>>>
+ Judah won't have to worry about
  + **Aram + Israel**: conquered by
+ **Assyria**: large kingdom further N.E.
  + land of the **Euphrates** river
+ known for fierceness and **cruelty**
  + past century: phase of **expansion**
+ God wields as **razor** shaves beard

---
<!-- .slide: <%= bg("Hans-wave-384385.jpg") %> -->
the Lord is bringing up against them 
the waters of the **River**, mighty and many, 
the king of **Assyria** and all his glory. 

And it will rise over all its **channels** 
and go over all its **banks**, 

[Isaiah 8:7 (ESV)](# "ref")
[Hans Braxmeier](https://pixabay.com/en/wave-sea-surf-swell-foam-spray-384385/ "caption")

>>>
+ **Euphrates** at flood stage
  + unstoppable
+ See it flood **enemies** of Judah
  + Aram + Israel
+ **relief**, even schadenfreude
  + but the waters aren't **stopping**:

---
<!-- .slide: <%= bg("unesco-Jerusalem_Damascus_Gate_108391.jpg") %>  -->
and it will sweep on into **Judah**, <br/>
it will **overflow** and pass on, <br/>
reaching even to the neck, 

and its outspread wings will fill <br/>
the breadth of your land, O **Immanuel**

[Isaiah 8:8 (ESV)](# "ref")
[Old City of Jerusalem, Damascus Gate. <br> Leila Maziz. &copy; UNESCO, CC-BY-SA 3.0 IGO](http://whc.unesco.org/en/documents/108391 "caption")

>>>
+ Ahaz + Judah thought were **safe**:
  + God **punished** their enemies
  + **vindicated** them
+ But instr of God's **wrath**
  + wouldn't **stop** there:
+ though 130yrs from fall of Samaria to Jeru
+ Jerusalem, the land of **Immanuel**
  + God **dwelling** with us
+ Surely God wouldn't judge **us**?
  + "good guys", call upon Name
  + temple of Lord, temple of Lord
+ In face of impending **disaster**,
  + Where did they turn?

---
<!-- .slide: <%= bg("unsplash-mKA3VavSuaI-dark_mtns.jpg") %> -->
And when they say to you, <br/>
“Inquire of the **mediums** and the **necromancers** [...]",

should not a people inquire of their **God**? 

[Isaiah 8:19 (ESV)](# "ref")

>>>
+ **mediums**: asking the dead for advice
  + **Saul** seek Samuel via Endor (1Sa28)
+ reversion to **youth**: life simple
  + had **mentor**, no responsibilities
+ but even **if** dead could speak
  + advice is still same:
+ we **know** we've done wrong
  + God's chastisement is **just**
+ we know what we need to **do**, 
  + are we **willing** to do it:

---
<!-- .slide: <%= bg("unsplash-1EbsMYrAu-s-bible_glasses.jpg") %> -->
To the **teaching** and to the **testimony**! <br>
If they will not speak according to this **word**, <br/>
it is because they have no **dawn**.

[Isaiah 8:20 (ESV)](# "ref")

>>>
+ **instr**: Torah, law, God's **design** for us 
  + **testimy**: witness, God's **work** in us
+ Read **every word** in Bible, live by it
  + Not performative **virtue**-signaling
  + Not **cherry**-picking what to obey
+ **Fathers**: take lead in **reading** Bible
  + **teach** kids acc. to God's Word
  + not **own** authority
+ Word as **lamp** (Ps 119:105): w/o it,
  + no **light** of dawn, 
  + only stumbling in the **dark**: 

---
<!-- .slide: <%= bg("unsplash-ndsRLeQBlsA-dark_clouds.jpg") %> -->
And when they are **hungry**, <br/>
they will be **enraged** <br>
and will speak **contemptuously** <br/>
against their **king** and their **God**,

[Isaiah 8:21b (ESV)](# "ref")

>>>
+ when disaster befalls **others**:
  + feel bad, but what can I do?
+ when it becomes **personal**:
  + when **revolutions** happen: angry
+ First **up**: curse king/God 
  + blame **authority** figures
+ e.g., little kid **runs** into a table:
  + "bad table", let's hit the table
+ teaches to **blame** others:
  + teacher, boss, spouse, immig, PM
+ ultimately, we blame **God**
  + for our **frustrations** + anger

---
<!-- .slide: <%= bg("unsplash-ndsRLeQBlsA-dark_clouds.jpg") %> -->
And they will look to the **earth**, <br/>
but behold, **distress** and darkness, <br/>
the gloom of anguish.

[Isaiah 8:22 (ESV)](# "ref")

>>>
+ but fighting against God is **futile**
+ **down**: darkness, distress
+ **this** is the world we live in!
  + **fear** of uncertain future
  + **anger** at authority + God
  + **frustration** at own futility
+ But **we** are not to be like that:

---
<!-- .slide: <%= bg("unsplash-bUhzlj3gGso-mtns_clouds.jpg") %> -->
Do not call **conspiracy** <br/>
all that this people calls conspiracy, <br/>
and do not **fear** what they fear, <br/>
nor be in dread. 

But the **Lord** of hosts, <br/>
him you shall **honor** as holy. <br/>
Let him be your **fear**, <br/>
and let him be your dread

[Isaiah 8:12-13 (ESV)](# "ref")
[Vietnam. Andreas Kind](https://unsplash.com/photos/bUhzlj3gGso "caption")

>>>
+ people of **Judah** living in **fear**
  + threat of unwinnable **war**
+ when **disaster** comes,
  + under **pressure**, sth has to **give**
+ What do we **compromise** on first
  + what do we **cling** to above all else?

---
<!-- .slide: data-background="white" -->
# How do I **respond** to severe **stress**? 

<!-- .element: class="r-fit-text" -->

>>>
+ e.g., as child: **hide** under covers
+ as parent: stew in silence until **explode**
+ "I **tried** asking nicely, tried praying"
  + "God, Your way didn't work, so
  + now I do it my way"
+ Where we **turn** in times of **crisis**
  + reflects what we truly **believe**

---
<!-- .slide: <%= bg("unsplash-TStNU7H4UEE-hand_sunset.jpg") %> class="outline" -->
## Hope in the Coming Messiah
1. The **Darkness** in our World [(8:19-22)](# "ref")
1. The **Light** Shining [(9:2-7)](# "ref")

>>>
+ starting in **Galilee**
+ portion **closest** to Aram + Assyria

---
<!-- .slide: <%= bg("unsplash-6tfO1M8_gas-forest_mist_beams.jpg") %> -->
The people who walked in **darkness** <br/>
have seen a great **light**;

those who dwelt in a land of deep **darkness**, <br/>
on them has **light** shone.

[Isaiah 9:2 (ESV)](# "ref")
[Chris Lawton](https://unsplash.com/photos/6tfO1M8_gas "caption")

>>>
+ where there was only
  + distress, darkness, anguish
+ Light of the **Creator**
  + "Let there be light"
+ Light of the **Saviour**
  + spoke into our hearts

---
<!-- .slide: <%= bg("unsplash-qVb-K2mFtDU-wheat.jpg") %> -->
You have **multiplied** the nation; <br/>
you have **increased** its joy;

they rejoice before you <br/>
as with joy at the **harvest**, <br/>
as they are glad when they divide the **spoil**.

[Isaiah 9:3 (ESV)](# "ref")
[Seth Schwiet](https://unsplash.com/photos/qVb-K2mFtDU "caption")

>>>
+ harvest/spoil: **abundance**
+ long-sought **respite** after toil
+ **sigh** of relief

---
<!-- .slide: <%= bg("unsplash-gtVrejEGdmM-chain.jpg") %> -->
For the **yoke** of his burden, <br/>
and the **staff** for his shoulder, <br/>
the **rod** of his oppressor, <br/>
you have broken as on the day of **Midian**.

[Isaiah 9:4 (ESV)](# "ref")
[Kerkini Lake, Greece. Eimaiokanenas, CC-BY-SA-4.0](https://commons.wikimedia.org/wiki/File:Kerkini_Lake_Port.jpg "caption")

>>>
+ yoke, staff, rod: **rule**, power
+ as in time of **Midian** (Jdg 6)
  + invaded like **locusts**
+ **victory** not by Gideon's might:

---
<!-- .slide: <%= bg("unsplash-iuqmGmst5Po-silh_hiking.jpg") %> -->
The Lord said to Gideon, <br/>
“The people with you are **too many** for me <br/>
to give the Midianites into their hand, 

lest Israel **boast** over me, <br/>
saying, ‘My **own hand** has saved me.’

[Judges 7:2 (ESV)](# "ref")
[Mittagberg, Germany. Tobias Mrzyk](https://unsplash.com/photos/iuqmGmst5Po "caption")

>>>
+ from 32k to just **300**
  + not bc **elite** soldiers
+ God **turned** Midianites against e.o.
  + all Israel came out to **pursue**
+ God brought about **end** to
  + what felt like interminable war:

---
<!-- .slide: <%= bg("unsplash-sM_ax7EU5Kw-boots.jpg") %> -->
For every **boot** of the tramping warrior <br/>
in battle tumult <br/>
and every garment rolled in **blood** <br/>
will be **burned** as fuel for the fire.

[Isaiah 9:5 (ESV)](# "ref")
[Neil Macc](https://unsplash.com/photos/sM_ax7EU5Kw "caption")

>>>
+ cleanup: accoutrements of **war**
  + burned after the battle is over
+ In Isaiah's time: 
  + Judah attacked by **Aram** + Israel
  + in turn conquered by **Assyria**
+ sin/pride: given to **Babylon**
  + Persia, Greece, Rome, ...
+ What will **stop** the cycle of war?

---
<!-- .slide: <%= bg("Rembrandt-Simeons_Song_of_Praise.jpg") %> -->
For to us a **child** is born, <br/>
to us a **son** is given; <br/>
and the **government** shall be upon his **shoulder**,

[Isaiah 9:6a (ESV)](# "ref")
[Rembrandt, "Simeons Song of Praise", 1631](https://www.mauritshuis.nl/en/explore/the-collection/artworks/simeons-song-of-praise-145/detailgegevens/ "caption")

>>>
+ not **rise** of commander 
  + or rags-to-riches undisc **hero**
+ but given from **birth**:
  + Christ reigns by **birthright**
+ **auth** to govern on **shoulder**:
  + cf v4 our shoulders bore 
  + **staff** of oppression
+ Christ's governance brings **justice**
+ Christmas not Christ's **beginning**:
  + In Jn8, establishing to Phar
  + that He is **greater** than Abraham:

---
<!-- .slide: <%= bg("unsplash-bUhzlj3gGso-mtns_clouds.jpg") %> -->
Jesus said to them, <br/>
“Truly, truly, I say to you, <br/>
before Abraham was, **I AM**.”

[John 8:58 (ESV)](# "ref")
[Vietnam. Andreas Kind](https://unsplash.com/photos/bUhzlj3gGso "caption")

>>>
+ tried to **stone** for blasphemy:
  + "I AM": God spoke of self to Moses
  + Jesus **is** God
+ didn't **start** at Christmas
  + didn't **end** at the ascension
+ **Work** of salv finished on cross, but
+ Promised **restoration** comes when
  + Christ **returns** and is
  + revealed in **power**:

---
<!-- .slide: <%= bg("pxhere-1094065-cross-silh.jpg") %> -->
and his name shall be called <br/>
Wonderful **Counselor**, Mighty **God**, <br/>
Everlasting **Father**, **Prince** of Peace.

[Isaiah 9:6b (ESV)](# "ref")
[pxhere](https://pxhere.com/en/photo/1094065 "caption")

>>>
+ Jesus **is** all these right now
  + and **will be** revealed as such
  + when He returns
+ **Counselor**: advisor, wisdom, guidance
  + consultants useless?
  + book knowledge / **theory**, or
  + extrapol from **experience**:
  + what I already know
+ but Christ is **wonderful**:
  + **beyond** our ken, 
  + wisdom of **Creator** God
  + revealed in incarnate **Word**
  + transcendent
+ **God**: not just **prophet**
  + God not **abstract**, far off
  + **Mighty**, capable to save
  + active, **involved** in world history
  + and our lives
+ **Father**: protect, guide:
  + **compassion**
  + earthly dad **limited**, sinful
  + Chr is **Eternal**, perfect
+ **Prince**: (not as son of king, but)
  + authority, governance, mgmt
  + true src of **Peace**: shalom

---
<!-- .slide: <%= bg("unsplash-E0e1NmVclRo-mist_lake.jpg") %> -->
Of the increase of his **government** <br/>
and of **peace** there will be no end, <br/>
on the throne of David and over his kingdom,

to establish it and to uphold it <br/>
with **justice** and with **righteousness** <br/>
from this time forth and forevermore.

The **zeal** of the Lord of hosts will do this.

[Isaiah 9:7 (ESV)](# "ref")
[Anna Goncharova](https://unsplash.com/photos/E0e1NmVclRo "caption")

>>>
+ **zeal**: fervor, jealousy 
  + for **hearts** that belong to Him
+ True, lasting **peace** is
  + not **superficial** politeness
    + ignores problems
  + not **giving in** to oppression
  + not vigilante **revenge**
+ Requires **justice**
  + restore what is **right**
+ Only when **Christ** is King
  + (not **Christians** running gov)
+ **Our** role is to **learn** Word
  + and **obey** it
+ Invitation back in ch2:

---
<!-- .slide: <%= bg("unsplash-TStNU7H4UEE-hand_sunset.jpg") %> -->
“**Come**, let us go up to the mountain of the Lord, <br/>
    to the house of the God of Jacob,

that he may **teach** us his ways <br/>
    and that we may **walk** in his paths.”

[Isaiah 2:3a (p.1/3) (ESV)](# "ref")

---
<!-- .slide: <%= bg("unsplash-TStNU7H4UEE-hand_sunset.jpg") %> -->
For out of Zion shall go forth the **law**, <br/>
    and the **word** of the Lord from Jerusalem.

He shall **judge** between the nations, <br/>
    and shall **decide** disputes for many peoples;

[Isaiah 2:3b-4a (p.2/3) (ESV)](# "ref")

---
<!-- .slide: <%= bg("unsplash-TStNU7H4UEE-hand_sunset.jpg") %> -->
and they shall beat their **swords** into plowshares, <br/>
    and their **spears** into pruning hooks;

nation shall not lift up sword against nation, <br/>
    neither shall they learn **war** anymore.

[Isaiah 2:4b (p.3/3) (ESV)](# "ref")

>>>
+ The final **end** to war/conflict
  + w/in own **hearts**+rl, and in **world**
+ **Disputes** settled by **God's judgment**
  + His **law** is proclaimed
+ When we **learn** His ways + **walk**
+ When all know: Christ is **King**

---
<!-- .slide: data-background="white" -->
# How shall I **wait** <br/> for Christ's **return**?

<!-- .element: class="r-fit-text" -->

>>>
+ **Passive** fatalism?
  + (does it even matter?)
+ Bitter **resentment**?
  + (like God even cares)
+ Proud **legalism**?
  + (I'm the only one doing sth!)
+ Or **confident** obedience?
+ I **know** Christ is returning
  + I **know** He sees and will judge
+ I am going to **trust** + obey
  + even when it hurts

---
<!-- .slide: <%= bg("unsplash-TStNU7H4UEE-hand_sunset.jpg") %> class="empty" -->
